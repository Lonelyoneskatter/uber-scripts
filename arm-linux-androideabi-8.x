...
export UBER_PATH=$DIR/out/arm-linux-androideabi-8.x;
...
echo ""
echo "${bldblu}Building your UBER arm-linux-androideabi-8.x Toolchain!!!${txtrst}"
echo ""
all1=$(date +%s.%N)
script -q $DIR/out/UBER-ANDROIDEABI-8.x.log -c "make 1>/dev/null $MAKE_FLAGS";
...
then
    ...
    echo "${bldgrn}░                                                                  ${txtrst}"
    echo ""
    echo "${bldgrn}Your UBER 8.x.x androideabi Toolchain has completed successfully!!! ${txtrst}"
    ...
    echo "${bldred}                               ▀                                 ${txtrst}"
    echo ""
    echo "${bldred}Error Log is found at:${txtrst}${red} $DIR/out/UBER-ANDROIDEABI-8.x.log ${txtrst}"
    echo ""
