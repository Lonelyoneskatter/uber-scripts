...
cd ../gcc/gcc-UBER && rm -rf * && git reset --hard && git fetch uu uber-5.x && git checkout FETCH_HEAD;
cd ../../
...
export UBER_PATH=$DIR/out/arm-eabi-5.x;
...
then
    ...
    echo "${bldgrn}░                                                                  ${txtrst}"
    echo ""
    echo "${bldgrn}Your UBER 5.x.x arm-eabi Toolchain has completed successfully!!!${txtrst}"
    ...
    echo "${bldred}                               ▀                                 ${txtrst}"
    echo ""
    echo "${bldred}Error Log is found at:${txtrst}${red} $DIR/out/UBER-ARM-EABI-5.x.log ${txtrst}"
    echo ""
